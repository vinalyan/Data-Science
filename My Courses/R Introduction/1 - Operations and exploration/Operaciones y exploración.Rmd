...
View(dplyr::filter(flights, carrier == "AA" & (distance >= 1300 & distance <= 1800)))
# --------------------------- #
})
```
...
|----------------|-----------------------------------------------------------------------|
| _pch_ | Especifica el símbolo a usar en el punto que representa al dato en la visualización. | 
